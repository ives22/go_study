...
// filePath: 日志存放路径;
// fileName: 日志文件名;
// splitType: 切割类型, size表示按照大小切割，并且maxSize必须传一个参数，time表示按照时间切割，默认1小时切割一次;
...
// 按照制定的日志大小，判断是否需要进行切割，如果大小超过设置的大小，则进行切割，如果没有超过，则不进行切割
...
// 按照小时进行切割，判断是否需要切割，如果当前时间点小时数与上一次切割时间点小时数相同则不切割，如果不同则切割
