...
func (f *FileLogger) splitFile(file *os.File) (*os.File, error) {
	...
	// 1. 关闭当前的日志文件
	file.Close()
	// 2. 备份一下 rename   xx.log  xx.log.bak202211081226
	os.Rename(logName, newLogName)
	// 3. 打开一个新的文件
...
func (f *FileLogger) log(lv LogLevel, format string, a ...interface{}) {
	if f.enable(lv) {
		...
		if f.checkSize(f.fileObj) {
			newFile, err := f.splitFile(f.fileObj) // 如果切割了，拿到新的日志文件
			if err != nil {
				return
			}
			f.fileObj = newFile
		...
		if lv >= ERROR {
			if f.checkSize(f.errFileObj) {
				newFile, err := f.splitFile(f.errFileObj) // 如果切割了，拿到新的日志文件
				if err != nil {
					return
				}
				f.errFileObj = newFile
			}
			// 如果要记录的日志大于等于ERROR级别，我还要在err日志文件中记录一份
